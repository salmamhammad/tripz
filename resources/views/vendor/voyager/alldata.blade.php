    ...
    <div class="page-content">
        ...
        <div class="analytics-container">
                ...
                <div class="panel panel-bordered ">
                    <h4 style="margin-right: 10px;text-align:center;"> Tous les ordres </h4>
                    <table  class="table table-hover dataTable no-footer" role="grid" aria-describedby="dataTable_info">
                        <thead>
                         <th>Nom d'agence </th>
                         <th>La somme totale </th>
                         <th>Le montant payé</th>
                         <th> Montant expiré</th>
                         <th>Montant en construction  </th>
                         <th> nombre de commandes en construction </th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @endphp
                                    @foreach ($hoteldemonds as $hoteldemond)
                                    @if ($user->id==$hoteldemond->userid)
                                    @php
                                         $total+= $hoteldemond->totalprice;
                                    @endphp
                                    @endif
                                    @endforeach
                                    @foreach ($visademonds as $visademond)
                                    @if ($user->id==$visademond->userid)
                                    @php
                                         $total+= $visademond->price;
                                    @endphp
                                    @endif
                                    @endforeach
                                ...
                                @if ($user->id==$demondrecord->userid)
                                @if ($demondrecord->operationtype=="se désister")
                                @if ($demondrecord->serviceid!=Null)
                                @php
                                     $totalpayd+= $demondrecord->amount;
                                @endphp
                                @endif
                                @endif
                                @endif
                           ...
                           @foreach ($hoteldemonds as $hoteldemond)
                           @if ($user->id==$hoteldemond->userid)
                           @if ($hoteldemond->status=="Termine")
                           @php
                                $totalcomplete+= $hoteldemond->totalprice;
                           @endphp
                           @endif    @endif
                           @endforeach
                           @foreach ($visademonds as $visademond)
                           ...
                           @php
                                $totalcomplete+= $visademond->price;
                           @endphp
                           @endif    @endif
                           @endforeach
                       ...
                       @foreach ($hoteldemonds as $hoteldemond)
                       @if ($user->id==$hoteldemond->userid)
                       @if ($hoteldemond->status=="En cours")
                       @php
                            $totalconstruction+= $hoteldemond->totalprice;
                       @endphp
                       @endif    @endif
                       @endforeach
                       @foreach ($visademonds as $visademond)
                       @if ($user->id==$visademond->userid)
                       @if ($visademond->status=="En cours")

                       @php
                            $totalconstruction+= $visademond->price;
                       @endphp
                       @endif    @endif
                       @endforeach
                ...
                <div class="panel panel-bordered ">
                    <h4 style="margin-right: 10px;text-align:center;"> Renseignements personnels </h4>
                    <div class="row">
                        <div class="col-md-12 col-sm-12" style="overflow-x: auto;">
                            <table  class="table table-hover dataTable no-footer" role="grid" aria-describedby="dataTable_info">
                             ...
                             <tr>
                                  ...
                                  <td>{{ $Agency->fax }}</td>
                                  <td>{{ $Agency->phone }}</td>
                                  <td>{{ $Agency->Licence }}</td>
                                  <td>{{ $Agency->mf }}</td>
                                  <td>{{ $Agency->Registre }}</td>
                                  @endif
                                  @endforeach
                             </tr>
                ...
                <div class="panel panel-bordered ">
                    <h4 style="margin-right: 10px;text-align:center;"> Service des visas  </h4>
                    <table  class="table table-hover dataTable no-footer" role="grid" aria-describedby="dataTable_info">
                        <thead>
                        ...
                        <tbody>
                           ...
                           <tr>
                           <td>{{  $visademond->country }}</td>
                           @foreach ($visas as $visa)
                             @if ($visa->id==$visademond->visaid)
                             <td>{{  $visa->type }}</td>
  
                             @endif

                           @endforeach
                           <td>{{  $visademond->numberfamily }}</td>
                           <td>{{  $visademond->nationality }}</td>
                           <td>{{  $visademond->docvoyager }}</td>
                           <td>{{  $visademond->datearrive }}</td>
                           <td>{{  $visademond->status }}</td>
                           <td>{{  $visademond->price }}</td>
                        </tr> 
                        @endif
                           @endforeach
                ...
                <div class="panel panel-bordered ">
                    <h4 style="margin-right: 10px;text-align:center;"> Service des hotels  </h4>
                    <table  class="table table-hover dataTable no-footer" role="grid" aria-describedby="dataTable_info">
                        <thead>
                        ...
                        <tbody>
                           ...
                           @foreach ($hotels as $hotel)
                             @if ($hotel->id==$hoteldemond->idhotel)
                             <td>{{  $hotel->name }}</td>
  
                             @endif

                           @endforeach
                           <td>{{  $hoteldemond->arrivedate }}</td>
                           <td>{{  $hoteldemond->departdate }}</td>
                           <td>{{  $hoteldemond->night }}</td>
                           <td>{{  $hoteldemond->numberroms }}</td>
                           <td>{{  $hoteldemond->adult }}</td>
                           <td>{{  $hoteldemond->child }}</td>
                           <td>{{  $hoteldemond->status }}</td>
                           <td>{{  $hoteldemond->totalprice }}</td>
                        </tr> 
                        @endif
                           @endforeach
